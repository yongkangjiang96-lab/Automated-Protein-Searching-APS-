...
def run_module_3_2_inference(input_fasta, model_path='Rostlab/ProstT5'):

    print(f"[Module 3.2] Initializing ProstT5 inference engine...")
    ...
    print(f"[Module 3.2] Processing sequences from {input_fasta}...")
    ...
    for r in records:
        seq_str = str(r.seq).replace("*", "") 
        prompt = "<AA2fold>" if seq_str.isupper() else "<fold2AA>"
        batch_strs_model.append(prompt + " " + " ".join(list(seq_str)))
        sequence_labels.append(r.id)
        sequence_strs.append(seq_str)

    print(f"[Module 3.2] Executing forward pass for {len(batch_strs_model)} sequences...")
    ...
    with torch.no_grad():
        token_representations = model(ids.input_ids, attention_mask=ids.attention_mask)
    
    final_representations = []
    for i in range(len(sequence_strs)):
        seq_len = len(sequence_strs[i])
        emb = token_representations.last_hidden_state[i, 1 : seq_len + 1].mean(dim=0)
        final_representations.append(emb.cpu().numpy())
    ...
    with open(output_pkl, 'wb') as f:
        pickle.dump({
            ...
            "representations": np.vstack(final_representations)
        }, f)
    
    print(f"-> Successfully encoded {len(sequence_labels)} sequences into {output_pkl}")
    return output_pkl
...
# - The score for proteins in the final database relies on Protrek.
#
# =============================================================================
